...
- **Demo resilience**: if the inference API is unavailable, `labeler.py` falls back to a
  safe default label so a case is never dropped. The demo call-extraction step
  (`demo.py`) does the same — it uses DO Gradient AI when a key is set, and scripted
